...
async function startRecordingLogs() {
  ...
  try {
    ...
    writeLog.alert('The log folder could not be emptied');
  ...
  try {
    ...
    writeLog.alert('The log recording could not start');
    writeLog.error(error);
  }
}
...
async function showFileLogs(options = {}) {
  ...
  let logs;
  try {
    logs = fs.readFileSync(consoleFilePath, 'utf8');
  } catch (err) {
    error = err;
    writeLog.error('No log file found');
  }
