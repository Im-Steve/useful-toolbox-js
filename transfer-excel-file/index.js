...
function exportExcelFile(data, filePath, options = {}) {
  ...
  try {
    ...
    if (displayLogs) {
      writeLog.success('Excel file exported successfully:', filePath);
    }
  } catch (err) {
    error = err;
    writeLog.error('An error occurred while exporting the Excel file:', filePath);
    writeLog.log(err);
    if (exitIfError) {
...
function importMultipleExcel(filePaths, options = {}) {
  ...
  if (displayLogs) {
    writeLog.info('Import multiple Excel files:', filePaths);
  }
  ...
  try {
    ...
    if (typeof filePaths === 'string') {
      ...
      if (displayLogs) {
        writeLog.log('files to import:', filesToImport);
      }
    }
