...
function exportJsonFile(data, filePath, options = {}) {
  ...
  try {
    ...
    if (displayLogs) {
      writeLog.success('JSON file exported successfully:', filePath);
    }
  } catch (err) {
    error = err;
    writeLog.error('An error occurred while exporting the JSON file:', filePath);
    writeLog.log(err);
    if (exitIfError) {
...
function importMultipleJson(filePaths, options = {}) {
  ...
  if (displayLogs) {
    writeLog.info('Import multiple JSON files:', filePaths);
  }
